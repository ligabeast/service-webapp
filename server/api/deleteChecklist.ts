...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // IDs als Platzhalter-Array für MySQL vorbereiten
    const ids = body.ids;

    // SQL-Abfrage mit Platzhaltern
    const sql = `
      DELETE FROM materialchecklist
      WHERE id IN (${ids.map(() => "?").join(",")}) AND assignedTo = ?;
    `;

    // Abfrage ausführen
    const [result] = await connection.execute(sql, [...ids, userId]);
